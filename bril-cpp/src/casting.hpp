...
union copy_construction_triviality_helper {
  ...
  copy_construction_triviality_helper(const copy_construction_triviality_helper&) =
      default;
  ~copy_construction_triviality_helper() = default;
};
...
struct is_trivially_copy_constructible
    : std::is_copy_constructible<detail::copy_construction_triviality_helper<T>> {};
template <typename T>
...
struct is_trivially_move_constructible
    : std::is_move_constructible<detail::move_construction_triviality_helper<T>> {};
template <typename T>
...
struct cast_retty {
  using ret_type =
      typename cast_retty_wrap<To, From,
...
struct cast_convert_val<To, FromTy*, FromTy*> {
  ...
  static typename cast_retty<To, FromTy*>::ret_type doit(const FromTy* Val) {
    return (typename cast_retty<To, FromTy*>::ret_type) const_cast<FromTy*>(Val);
  }
};
...
struct CastIsPossible {
  static inline bool isPossible(const From& f) {
    return isa_impl_wrap<To, const From,
...
/// always be possible.
template <typename To, typename From>
struct CastIsPossible<To, From, std::enable_if_t<std::is_base_of<To, From>::value>> {
...
using SelfType = std::conditional_t<std::is_same<OptionalDerived, void>::value, Default,
                                    OptionalDerived>;
}  // namespace detail
...
struct ValueFromPointerCast
    : public CastIsPossible<To, From*>,
      ...
      public DefaultDoCastIfPossible<
          To, From*, detail::SelfType<Derived, ValueFromPointerCast<To, From>>> {
...
struct UniquePtrCast : public CastIsPossible<To, From*> {
  ...
  using CastResultType =
      std::unique_ptr<std::remove_reference_t<typename cast_retty<To, From>::ret_type>>;
...
template <typename To, typename From, typename Derived = void>
struct OptionalValueCast : public CastIsPossible<To, From>,
                           public DefaultDoCastIfPossible<
...
struct ConstStrippingForwardingCast {
  ...
  using NonConstFrom =
      std::conditional_t<std::is_pointer<From>::value, DecayedFrom*, DecayedFrom&>;
...
struct CastInfo : public CastIsPossible<To, From> {
  ...
  static inline CastReturnType doCast(const From& f) {
    return cast_convert_val<To, From, typename simplify_type<From>::SimpleType>::doit(
        const_cast<From&>(f));
  }
...
struct CastInfo<To, From, std::enable_if_t<!is_simple_type<From>::value>> {
  ...
  static inline bool isPossible(From& f) {
    return SimplifiedSelf::isPossible(simplify_type<From>::getSimplifiedValue(f));
  }
  ...
  static inline decltype(auto) doCastIfPossible(From& f) {
    return SimplifiedSelf::doCastIfPossible(simplify_type<From>::getSimplifiedValue(f));
  }
};
