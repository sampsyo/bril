...
template <typename T, bool TriviallyCopyable>
T* SmallVectorTemplateBase<T, TriviallyCopyable>::mallocForGrow(size_t MinSize,
                                                                size_t& NewCapacity) {
...
extern template class bril::SmallVectorBase<uint32_t>;
#if SIZE_MAX > UINT32_MAX
extern template class bril::SmallVectorBase<uint64_t>;
#endif
...
template <typename T>
inline void swap(bril::SmallVectorImpl<T>& LHS, bril::SmallVectorImpl<T>& RHS) {
  LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N>
inline void swap(bril::SmallVector<T, N>& LHS, bril::SmallVector<T, N>& RHS) {
  LHS.swap(RHS);
}
