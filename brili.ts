...
/** Inverts `typeCheck`. */
function inferType(val: Value): bril.Type {
  ...
  throw error(`unknown type for value ${val}`);
}
...
function getArgumentRaw(instr: bril.Operation, env: Env, index: number): Value {
  let args = instr.args || [];
  if (args.length <= index) {
    throw error(`${instr.op} expected at least ${index + 1} arguments; got ${args.length}`);
  }
  return get(env, args[index]);
}
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  switch (instr.op) {
    ...
    case "bitcast": {
      // These two functions are stolen from https://gist.github.com/JianLoong/1c7ac99e588182ccc20dc0320fde1ad7

      // Converts number into a 64-bit binary using its IEEE764 Representation
      function convertNumberToBinary(num: number): bigint {
        let str = "";
      ...
      function convertBinaryToNumber(bits: bigint): number {
        let str = bits.toString(2).padStart(64, " ");
        // Convert back to Uint8Array.
        if (str.length !== 64)
          throw new Error(`Binary ${str} cannot be converted because the length is not 64.`)
        const arr = [];
      ...
      let value = getArgumentRaw(instr, state.env, 0);
      let value_type = inferType(value);
      let destination_type = instr.type;
      if (value_type == destination_type) {
        state.env.set(instr.dest, value);
        return NEXT;
      }

      if (value_type === "float" && destination_type === "int") {
        state.env.set(instr.dest, convertNumberToBinary(value as number));
      } else if (value_type === "int" && destination_type === "float") {
        state.env.set(instr.dest, convertBinaryToNumber(value as bigint));
      } else {
        throw error(`invalid bit cast between ${value_type} and ${destination_type}`);
      }
