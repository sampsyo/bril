...
function decrementRefCount(variable: Value | undefined, state: State) {
  // ignore if the variable is not a pointer
  ...
  if (refCount) {
    ...
    if (refCount == 0) {
      ...
      state.refcount.set(base, refCount);
    }
  }
}

function incrementRefCount(ptr: Value | undefined, state: State) {
  // ignore if the variable is not a pointer
  if (!ptr || typeof ptr !== "object" || ptr instanceof BigInt) {
    return;
  }
  state.refcount.set(ptr.loc.base, (state.refcount.get(ptr.loc.base) || 0) + 1);
}
...
type State = {
  ...
  // maps base (memory location) to its number of references
...
function evalCall(instr: bril.Operation, state: State): Action {
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    // need to increment the ref count of the parameters 
    incrementRefCount(value, state);
  }
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  switch (instr.op) {
    ...
    case "id": {
      ...
      // MUST increment the reference on the RHS BEFORE we decrement or
      // we might accidentally free LHS if RHS and LHS are aliases
      incrementRefCount(val, state);
      decrementRefCount(state.env.get(instr.dest), state);
    ...
    case "free": {
      // turn free instructions to no-ops
