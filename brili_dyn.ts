...
type State = {
  ...
  // For dynamic compilers assignment: list of insns that ran + labels we have visited
  tracing: boolean,
  ran_insns: bril.Instruction[] | null
  seen_labels: Set<bril.Ident> | null
  awaiting_return_vals: bril.Ident[] | null // for interprodedural tracing, invariant is if len > 0 then we are in a call instruction
}
...
function evalCall(instr: bril.Operation, state: State): Action {
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    // if tracing, add instruction that assigns param to id arg - these replace the call insn
    if (state.tracing) {
      let id_arg: bril.Instruction = { op: "id", dest: params[i].name, args: [args[i]], type: params[i].type }
      state.ran_insns!.push(id_arg)
    }

  }
  // if has destination, push onto stack of awaiting returns
  if ('dest' in instr && state.tracing) {
    state.awaiting_return_vals!.push(instr.dest)
  }
  ...
  let newState: State = {
    ...
    // copy over all tracing stuff
    tracing: state.tracing,
    ran_insns: state.ran_insns,
    seen_labels: state.seen_labels,
    awaiting_return_vals: state.awaiting_return_vals
  }
  let retVal = evalFunc(func, newState);
  state.icount = newState.icount;
  if (state.tracing) {
    state.ran_insns = newState.ran_insns // update to include anything new
    state.seen_labels = newState.seen_labels
  }
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  if (state.tracing && instr.op != "jmp" && instr.op != "br" && instr.op != "call" && instr.op != "ret") { // we don't want to add jump or branch or call instructions 
    state.ran_insns!.push(instr)
  ...
  switch (instr.op) {
    ...
    case "br": {
      ...
      if (state.tracing) {
        let guard_instr: bril.Instruction;
        if (cond) {
          guard_instr = { op: "guard", args: [instr.args![0]], labels: [getLabel(instr, 1)] }
        } else {
          // need to change the op to false, so insert an instruction that does that
          let not_cond = "not_" + instr.args![0];
          let extra_instr: bril.Instruction = { op: "not", dest: not_cond, type: "bool", args: [instr.args![0]] };
          //console.error(JSON.stringify(extra_instr));        
          state.ran_insns!.push(extra_instr);
          guard_instr = { op: "guard", args: [not_cond], labels: [getLabel(instr, 0)] }
        }
        state.ran_insns!.push(guard_instr);
        //console.error(JSON.stringify(guard_instr));
      }
      if (cond) {
        return { "action": "jump", "label": getLabel(instr, 0) };
      } else {
    ...
    case "ret": {
      ...
      if (args.length == 0) {
        ...
        if (state.tracing && state.awaiting_return_vals!.length > 0) { //short circuits, if first is true then awaiting_return_vals is not null
          let awaiting_val = state.awaiting_return_vals!.pop()!
          let typ: bril.PrimType;
          switch (typeof val) {
            case 'string': {
              typ = 'char'
              let id_instr: bril.Instruction = { op: "id", dest: awaiting_val, args: args, type: typ }
              state.ran_insns!.push(id_instr)
            }
            case 'number': {
              typ = 'float'
              let id_instr: bril.Instruction = { op: "id", dest: awaiting_val, args: args, type: typ }
...
function evalFunc(func: bril.Function, state: State): Value | null {
  for (let i = 0; i < func.instrs.length; ++i) {
    ...
    if ('op' in line) {
      ...
      if ('label' in action) {
        ...
        for (i = 0; i < func.instrs.length; ++i) {
          ...
          if ('label' in sLine && sLine.label === action.label) {
            ...
            // if this is a label we have seen before AND we are not in the middle of a call instr
            // then stop tracing and write the trace to a file
            // ADD A JUMP INSTRUCTION AT THE END OF THE TRACE TO THE NEXT LABEL
            if (state.tracing && state.awaiting_return_vals!.length == 0) {
              if (state.seen_labels!.has(sLine.label)) {
                let last_instr: bril.Instruction = { op: "jmp", labels: [sLine.label] }
                state.ran_insns!.push(last_instr)
              }
              let traced_fn: bril.Function = { name: "main_trace", instrs: state.ran_insns! }
              let trace_prog: bril.Program = { functions: [traced_fn] }
              console.error(JSON.stringify(trace_prog))
              state.tracing = false
            }
      ...
      if (state.tracing) {
        state.seen_labels!.add(line.label);
      }
    }
  }
...
function evalProg(prog: bril.Program) {
  ...
  // er i guess i'll make dynamic an argument too??
  let didx = args.indexOf('-dyn');
  let ran_insns: bril.Instruction[] | null = null;
  let seen_labels: Set<bril.Ident> | null = null;
  let await_vals: bril.Ident[] | null = null;
  let tracing = false;
  if (didx > -1) {
    tracing = true;
    args.splice(didx, 1);
    ran_insns = [];
    seen_labels = new Set<bril.Ident>();
    await_vals = [];
  }
  ...
  let state: State = {
    ...
    awaiting_return_vals: await_vals
  }
  evalFunc(main, state);
  ...
  // if we execute this it means we traced the whole program
  if (state.tracing) {
    let final_jmp: bril.Instruction = { op: "jmp", labels: [".trace_end"] }
    state.ran_insns!.push(final_jmp)
    let traced_fn: bril.Function = { name: "main_trace", instrs: state.ran_insns! }
    let trace_prog: bril.Program = { functions: [traced_fn] }
    console.error(JSON.stringify(trace_prog))
    state.tracing = false
